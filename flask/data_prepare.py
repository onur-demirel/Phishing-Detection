...
for root, dirs, files in os.walk(benign_data_path):
    for file in files:
        if file.endswith("html.txt"):
            ...
            total += 1
            ben_misled += 1
            shutil.copy(os.path.join(root, file), benign_misleading_path + "\\" + root[len(benign_data_path) + 1:].replace(".","_") + "_" + file)
...
for root, dirs, files in os.walk(misleading_data_path):
    for file in files:
        if file.endswith("html.txt"):
            ...
            total += 1
            ben_misled += 1
            shutil.copy(os.path.join(root, file), benign_misleading_path + "\\" + root[len(misleading_data_path) + 1:].replace(".","_") + "_" + file)
...
for root, dirs, files in os.walk(phishing_data_path):
    for file in files:
        if file.endswith("html.txt"):
            ...
            total += 1
            phis += 1
            shutil.copy(os.path.join(root, file), phishing_path + "\\" + root[len(phishing_data_path) + 1:].replace(".","_") + "_" + file)
